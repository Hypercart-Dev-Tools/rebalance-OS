...
- `Unified UI refresh + restart (system-wide)` — two endpoints on the always-on pulse-server (`POST /api/refresh`, `POST /api/restart`) that every UI (pulse dashboard, Focus 5 app) calls, so no source goes stale and no manual terminal sync is ever needed again. Reminders refresh routes through the signed helper (EventKit, no FDA) since the launchd server can't hold Full Disk Access. **Plan written 2026-06-27** after a /ponytail review; 4 phases (FDA-free reminders read → real `/api/refresh` → `/api/restart` + button → Focus 5 wiring), all with QA gates. **Next:** Phase 1. → [PROJECT/2-WORKING/UNIFIED-REFRESH-RESTART.md](PROJECT/2-WORKING/UNIFIED-REFRESH-RESTART.md)
- `Apple Reminders unified integration plan` — read-only `apple_reminders` SQLite source, collector-first architecture with explicit QA gates. **Phases 0 + 1 complete (2026-06-25):** FDA/TCC gate passed in VS Code, active store discovered deterministically, WAL-safe snapshot extractor shipped in `src/rebalance/ingest/apple_reminders.py`, live-verified read-only extraction (9010 reminders, 8 lists, ~0.18s) with 9 unit tests. **Next:** Phase 2 collector registration + storage; optional deferred write-back spike is tracked as a separate future Phase 5, not part of the core delivery path. → [PROJECT/2-WORKING/APPLE-REMINDERS-UNIFIED-PLAN.md](PROJECT/2-WORKING/APPLE-REMINDERS-UNIFIED-PLAN.md)
