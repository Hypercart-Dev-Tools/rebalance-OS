...
|---|---|
| **Focus 5 Float reminders drawer widened to Apple + Obsidian (2026-06-30).** The floating Mac app now shows explicit Apple Reminders plus an Obsidian Reminders block sourced from vault `0. Goals.md`, with exact-line checkbox completion and precise load diagnostics; build + install path verified in `/Applications`. | **Three path-disjoint build lanes** (see [the parallel queue](PROJECT/2-WORKING/MARATHON-2026-06-27.md)): (1) VS Code "focus-if-open" repo links — Phase 1 (Mac app); (2) Unified refresh **v1** (signed helper `list-active` → `/api/refresh` → reminders column); (3) Focus 5 standalone **App Store Phase 0** spike. _Prior track:_ front-door v0.41.1 — `migrate-secrets` on the ~2 remaining Macs, then a `development` → `main` PR. |

## Ledger

### In progress

- `Client auto-discovery` — **Active, Phase 1 (v1 deterministic, 2026-06-30).** Signal pipeline auto-discovers projects into a discrete bucket before synthesis but never discovers clients (config-only metadata). Lazy design: a client is an *attribute of a project*, not a new entity — store inferred `client` in the existing `project_registry.custom_fields`, derive buckets as a GROUP BY on read, owner-as-client deterministic spine + Gemini gap-fill (Phase 2) for the rest. No new table/lifecycle/MCP tool in v1 (~50 lines, 3 existing files + 1 test). Phase 0 design accepted, promoted to `2-WORKING`; building Edits 1–3 + test. → [PROJECT/2-WORKING/CLIENT-AUTO-DISCOVERY.md](PROJECT/2-WORKING/CLIENT-AUTO-DISCOVERY.md)
