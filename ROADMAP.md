...
- `Apple Reminders unified integration plan` — read-only `apple_reminders` SQLite source, collector-first architecture with explicit QA gates. **Phases 0–3 core complete (2026-06-27):** access/schema spike, read-only extractor + snapshot pipeline, registered `apple_reminders` collector + storage (list-scoped to the default `Reminders` list, configurable; verified live via `refresh_index` — 8147 rows, idempotent), and the `list_apple_reminders` read accessor (safe-by-default active-only, pure-sqlite3, 26 tests). Deferred: product-surface wiring, Phase 4 hardening, notes/sections decode, snapshot perf wins. → [PROJECT/2-WORKING/APPLE-REMINDERS-UNIFIED-PLAN.md](PROJECT/2-WORKING/APPLE-REMINDERS-UNIFIED-PLAN.md)
