...
- `Apple Reminders unified integration plan` — read-only `apple_reminders` SQLite source, collector-first architecture with explicit QA gates. **Phases 0–2 complete (2026-06-27):** access/schema spike, read-only extractor + snapshot pipeline, and the registered `apple_reminders` collector + storage (list-scoped to the default `Reminders` list, configurable; verified live via `refresh_index` — 8147 rows, idempotent). Notes/sections decode + Phase 3 query surface deferred. → [PROJECT/2-WORKING/APPLE-REMINDERS-UNIFIED-PLAN.md](PROJECT/2-WORKING/APPLE-REMINDERS-UNIFIED-PLAN.md)
