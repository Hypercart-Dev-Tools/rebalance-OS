...
- `Watch-list coverage guard` — the watched-repos set is a recomputed union with no persisted history, so a repo held only by a rolling window (`activity` 14d / `pushed`) drops off the roster with no trace (the `BinoidCBD/LTVera-Pandas` "fell out" report — now confirmed `watched_and_fresh`). **Phase 1 shipped (2026-06-26):** additive migration `0009` + isolated `watchlist_guard.py` (pure `diff_watched_set`/`classify_removal` + `snapshot_and_detect` single writer) hooked at the end of `_refresh_github` (clean-sync only) + `log_watched_repos_reduced` helper; all 4 agy `[Should]` baked in (canonical `since_days=14`, clean-sync guard, ignore-list suppression, 30-day pruning). 16 new tests; suite 1137 green; `doctor` clean; live baseline 59 watched / 24 durable-intent. **Phase 2 (web `_EVENT_BADGE` + `/auth-log` render assertion) next.** Reuses focus5 snapshot pattern / `auth_log` surface / `github-sync` piggyback — no new screen/job. → [PROJECT/2-WORKING/WATCHLIST-COVERAGE-GUARD.md](PROJECT/2-WORKING/WATCHLIST-COVERAGE-GUARD.md)
