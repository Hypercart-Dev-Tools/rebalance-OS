...
- `Watch-list coverage guard` — the watched-repos set is a recomputed union with no persisted history, so a repo held only by a rolling window (`activity` 14d / `pushed`) drops off the roster with no trace (the `BinoidCBD/LTVera-Pandas` "fell out" report — now confirmed `watched_and_fresh`). **Plan scaffolded (2026-06-26), Phase 1 next:** additive snapshot table (migration `0009`) + pure `diff_watched_set` + a single github-sync writer that emits a `watched_repos_reduced` event to the existing `/auth-log` screen on a concerning reduction. Reuses the focus5 snapshot pattern, the `auth_log` surface, and the `github-sync` piggyback — no new screen/job. → [PROJECT/2-WORKING/WATCHLIST-COVERAGE-GUARD.md](PROJECT/2-WORKING/WATCHLIST-COVERAGE-GUARD.md)
