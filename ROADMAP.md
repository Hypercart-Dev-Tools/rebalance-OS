...
- `Client auto-discovery` — **SKETCH (1-INBOX intake, 2026-06-30).** Signal pipeline auto-discovers projects into a discrete bucket before synthesis but never discovers clients (config-only metadata). Lazy design: a client is an *attribute of a project*, not a new entity — store inferred `client` in the existing `project_registry.custom_fields`, derive buckets as a GROUP BY on read, owner-as-client deterministic spine + Gemini gap-fill for the rest. No new table/lifecycle/MCP tool in v1 (~50 lines, 3 existing files + 1 test). Pending design accept → promote to `2-WORKING`. → [PROJECT/1-INBOX/CLIENT-AUTO-DISCOVERY.md](PROJECT/1-INBOX/CLIENT-AUTO-DISCOVERY.md)
