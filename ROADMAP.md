...
- `Apple Reminders unified integration plan` — read-only `apple_reminders` SQLite source, collector-first architecture with explicit QA gates. **Phases 0–4 complete (2026-06-27):** access/schema spike, read-only extractor + snapshot pipeline, registered `apple_reminders` collector + storage (default `Reminders` list, configurable; live-verified via `refresh_index`, idempotent), `list_apple_reminders` read accessor (safe-by-default) **+ read-only Apple Reminders column on the pulse "Today" dashboard** (:8767), and hardening — schema-drift health (`doctor` + `index_status`), schema fingerprint, FDA/drift runbook. 31 tests. Deferred by choice: cross-version validation (needs 2nd macOS), notes/sections decode, snapshot perf wins. → [PROJECT/2-WORKING/APPLE-REMINDERS-UNIFIED-PLAN.md](PROJECT/2-WORKING/APPLE-REMINDERS-UNIFIED-PLAN.md)
