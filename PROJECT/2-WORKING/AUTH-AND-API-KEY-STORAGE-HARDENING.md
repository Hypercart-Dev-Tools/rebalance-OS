...
- [~] Introduce one secret-storage module that owns:
  ...
  Progress: module + primitives shipped in [src/rebalance/ingest/secret_store.py](/Users/noelsaw/Documents/rebalance-OS/src/rebalance/ingest/secret_store.py:1). **GitHub + Figma loaders now call it** via the shared dual-store helpers ([config.py](/Users/noelsaw/Documents/rebalance-OS/src/rebalance/ingest/config.py:111), additive: keyring → secret-store → config). Sleuth + Google OAuth wiring and migration helpers still to come.
