...
|---|---|
| **Phase 1 complete (2026-06-10):** explicit `kind` taxonomy (raw_source / derived_scan / projection / export); `all` narrowed to raw sources with a named default recipe (raw + code + semantic + sync); back-compat kept; taxonomy recorded in `AGENTS.md` (ARCHITECTURE.md deferred — regenerated by ask-self ingest). Commits `8f1c447`, `f731dbd`, `ff9a8ae`; contract tests green. | **Phase 2:** consolidate user-facing write paths — route the ~12 CLI/MCP leaf-ingest bypasses through the orchestrator or one source-owned helper (flips the no-bypass contract test green). Most are *partial/granular* ops, so classify per-site first. |
