...
|---|---|
| **Planning + QA review (2026-07-19).** Root-caused the durability gap in the CLIO exporter and restructured the plan into 4 phased, checklisted work units with per-phase QA gates. Ran an automated **agy** review (`relay-xyz`, thread `.xyz/relay-system/2026-07-19/clio-durable-plan-qa.md`) against GUIDING-PRINCIPLES.md: **8/8 review heuristics Pass**, verdict *Changes requested* on 4 refinements (2 Should, 2 Nit) — all **accepted and applied**: full-block extraction in Phase 2, subprocess-free inline `session_id+timestamp` IDs in Phase 1, explicit `<!--`-line pre-filter in the Swift reader, and a scope classification (export/projection, not `--source all` eligible). | Execute **Phase 0** (spike): reproduce the three failure modes and pin the real conflict-file naming on this sync stack, writing findings back into this doc before its QA gate can pass. |
