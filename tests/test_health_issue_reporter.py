...
# ===========================================================================
# 10. Stable check-id dedup (GH-139)
# ===========================================================================

class TestStableCheckId(unittest.TestCase):

    def test_extract_check_id_from_body(self) -> None:
        body = "## heading\n<!-- rebalance-check-id: my-check -->"
        self.assertEqual(hr._extract_check_id(body), "my-check")

    def test_extract_check_id_returns_none_on_legacy_body(self) -> None:
        body = "## Rebalance health: `my-check`\n**Status:** `FAIL`"
        self.assertIsNone(hr._extract_check_id(body))

    def test_extract_check_id_returns_none_on_empty_body(self) -> None:
        self.assertIsNone(hr._extract_check_id(""))

    def test_issue_body_contains_check_id_comment(self) -> None:
        body = hr._issue_body("db-check", "fail", "detail", "fix it", "rebalance-doctor")
        self.assertIn("<!-- rebalance-check-id: db-check -->", body)

    def test_issue_body_still_starts_with_seen_marker(self) -> None:
        body = hr._issue_body("db-check", "fail", "detail", "fix it", "rebalance-doctor")
        self.assertTrue(body.startswith("> **Seen:** 1×"),
                        f"Seen marker must still be first; got: {body[:80]!r}")

    def test_list_health_issues_indexes_by_check_id(self) -> None:
        body = hr._issue_body("my-check", "fail", "d", "h", "rebalance-doctor")
        fake_issue = {"title": "health: my-check", "number": 1, "body": body}

        with patch.object(hr, "_request", return_value=[fake_issue]):
            issues = hr.list_health_issues("token", "org/repo", state="open")

        self.assertIn("my-check", issues)
        self.assertNotIn("health: my-check", issues)

    def test_list_health_issues_falls_back_to_title_for_legacy_issues(self) -> None:
        legacy_body = "## Rebalance health: `my-check`\n> **Seen:** 1×"
        fake_issue = {"title": "health: my-check", "number": 1, "body": legacy_body}

        with patch.object(hr, "_request", return_value=[fake_issue]):
            issues = hr.list_health_issues("token", "org/repo", state="open")

        self.assertIn("health: my-check", issues)
        self.assertNotIn("my-check", issues)

    def test_stable_id_lookup_before_title_fallback(self) -> None:
        """Main loop should match an issue via check-id even when the title key differs."""
        body = hr._issue_body("pulse collector:device-a", "warn", "d", "h", "rebalance-doctor")
        # Simulate an issue indexed by check-id (as list_health_issues would do)
        open_issues = {"pulse collector:device-a": {"title": "health: pulse collector:device-a",
                                                     "number": 7, "body": body}}
        check_name = "pulse collector:device-a"
        title = f"{hr.ISSUE_TITLE_PREFIX} {check_name}"

        already_open = open_issues.get(check_name) or open_issues.get(title)
        self.assertIsNotNone(already_open)
        self.assertEqual(already_open["number"], 7)


# ===========================================================================
# 11. Detail block refresh (GH-139)
# ===========================================================================

class TestDetailRefresh(unittest.TestCase):

    def test_refresh_detail_block_replaces_detail(self) -> None:
        body = hr._issue_body("my-check", "fail", "old detail", "fix it", "rebalance-doctor")
        updated = hr._refresh_detail_block(body, "new detail")
        self.assertIn("new detail", updated)
        self.assertNotIn("old detail", updated)

    def test_refresh_detail_block_no_match_returns_original(self) -> None:
        body = "## heading\nsome content without a detail block"
        updated = hr._refresh_detail_block(body, "new detail")
        self.assertEqual(updated, body)

    def test_refresh_preserves_other_body_sections(self) -> None:
        body = hr._issue_body("my-check", "fail", "old detail", "fix it", "rebalance-doctor")
        updated = hr._refresh_detail_block(body, "new detail")
        self.assertIn("## Rebalance health:", updated)
        self.assertIn("**Status:**", updated)
        self.assertIn("<!-- rebalance-check-id:", updated)
        self.assertIn("> **Seen:** 1×", updated)

    def test_open_issue_detail_refreshed_on_update(self) -> None:
        """When an issue is already open, the PATCH body must contain the new detail."""
        ts = _now_str()
        existing_body = hr._issue_body("my-check", "fail", "old detail", "fix it", "rebalance-doctor")
        ...
        with patch.object(hr, "_request", side_effect=_fake_request):
            ...
            new_count = hr.parse_occurrence_count(current_body) + 1
            new_body = hr.set_occurrence_count(current_body, new_count, ts)
            new_body = hr._refresh_detail_block(new_body, "new detail")
            hr.update_issue_body("token", "org/repo", 42, new_body, dry_run=False)

        patch_payload = patched_calls[0][2]
        self.assertIn("new detail", patch_payload["body"])
        self.assertNotIn("old detail", patch_payload["body"])

    def test_detail_refresh_is_idempotent(self) -> None:
        """Refreshing with the same detail twice leaves the body stable."""
        body = hr._issue_body("my-check", "fail", "same detail", "h", "rebalance-doctor")
        once = hr._refresh_detail_block(body, "same detail")
        twice = hr._refresh_detail_block(once, "same detail")
