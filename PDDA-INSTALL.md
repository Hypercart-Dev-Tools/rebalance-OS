...
> Note: `pdda-run.sh` invokes `pdda-check-roadmap-coverage.sh` and `pdda-check-changelog.sh`
> as part of its deterministic suite, so both are part of the canonical set. Omitting them
> yields a noisy run and silently skips the `pdda-doc-ready.sh` readiness review.
